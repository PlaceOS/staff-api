...
describe Bookings do
  ...
  it "#create and #update should respect booking limits when booking on behalf of other users" do
    ...
    first_booking = Context(Bookings, JSON::Any).response("POST", "#{BOOKINGS_BASE}/",
      body: %({"asset_id":"first_desk","booking_start":#{starting},"booking_end":#{ending},"booking_type":"desk", "user_id": "bob@example.com", "user_email": "bob@example.com"}),
      headers: Mock::Headers.office365_guest, &.create)[1].as_h
    first_booking["asset_id"].should eq("first_desk")

    second_booking = Context(Bookings, JSON::Any).response("POST", "#{BOOKINGS_BASE}/",
      body: %({"asset_id":"second_desk","booking_start":#{starting},"booking_end":#{ending},"booking_type":"desk", "user_id": "bob@example.com", "user_email": "bob@example.com"}),
      headers: Mock::Headers.office365_guest, &.create)[1].as_h
    second_booking["asset_id"].should eq("second_desk")

    # Fail to create booking due to limit
    not_created = Context(Bookings, JSON::Any).response("POST", "#{BOOKINGS_BASE}/",
      body: %({"asset_id":"third_desk","booking_start":#{starting},"booking_end":#{ending},"booking_type":"desk", "user_id": "bob@example.com", "user_email": "bob@example.com"}),
      headers: Mock::Headers.office365_guest, &.create)[0]
    not_created.should eq(409)

    # Create third booking at a different time
    third_booking = Context(Bookings, JSON::Any).response("POST", "#{BOOKINGS_BASE}/",
      body: %({"asset_id":"third_desk","booking_start":#{different_starting},"booking_end":#{different_ending},"booking_type":"desk", "user_id": "bob@example.com", "user_email": "bob@example.com"}),
      headers: Mock::Headers.office365_guest, &.create)[1].as_h
    third_booking["asset_id"].should eq("third_desk")
